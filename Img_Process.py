...
def blackBorder():
    imgGray = cv2.imread('input1.png', 0)
...
def contourStar(img):
    ...
    for cnt in contours:
        ...
        cv2.drawContours(imgCT, [cnt], -1, (0, 0, 255), 2)
    return imgCT
...
def contourRec(img):
    ...
    for cnt in contours:
        x, y, w, h = cv2.boundingRect(cnt)
        cv2.rectangle(imgCT, (x, y), (x + w, y + h), (0, 255, 0), 2)
    return imgCT
...
def sharpen(img):
    ...
    imgSharped = cv2.putText(imgSharped, f'519K0078, 520K0328', (136, 386),
...
if __name__ == "__main__":
    ...
    for i in range(len(greenStars)):
        ...
        cv2.imwrite('rectangle_stars.png', contourRec(img))
        # show(contourRec(img))
        cv2.imwrite('sharped_image.png', sharpen(img))
